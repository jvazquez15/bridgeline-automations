...
export default function Home() {
	...
	return (
		<div className="w-full min-h-screen flex flex-1 items-start justify-center bg-zinc-50 font-sans text-zinc-900">
			<div className="flex h-full w-full flex-col gap-6 p-8 max-w-5xl">
				<header className="flex items-center justify-between">
					<h1 className="text-2xl font-semibold">Bridgeline Automation&apos;s</h1>
				...
				<section>
					<div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
						{automationsFields.map((automation) => 
							<AutomationCard key={automation.id} module={automation} onOpen={handleOpenModule} />
						)}
					</div>
				</section>
